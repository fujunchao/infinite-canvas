...
+ [优化] 保留新版单节点多图数据结构，候选图支持拖动、双击或点击按钮独立到画布，并立即使用普通图片节点的完整交互能力。

## v0.15.3 - 2026-08-09

+ [修复] 画布多图操作按钮根据节点宽度自适应显示，窄幅竖图使用带悬浮提示的纯图标，避免按钮文字被裁切或相互遮挡。

## v0.15.2 - 2026-08-09

+ [调整] 合并上游 `v0.15.1`，同步单节点多图生成、画布选择与移动模式、提示词放大编辑、本地存储统计以及 Agent 内联 Skill 和画布引用等功能。
+ [调整] 跟随上游将开源协议切换为 MIT，删除 CLA，并保留当前 fork 的仓库、GHCR、Docker Compose、版本检查和官方插件注册表配置。
+ [修复] 升级旧版多图画布数据时保留全部图片、主图、存储元数据和外部连线，避免旧批次在新架构下丢失关系。
+ [修复] 新版 Agent 消息元数据缺失时继续读取旧 IndexedDB 附件预览，保留升级前的历史附件展示。
...
+ [修复] 修正文档站中英文 locale 合并错误，统一快速开始、Render、Docker、CLA、安全策略、更新日志和 Codex 插件安装页面的语言与内容。
+ [修复] 中文部署和插件文档全部改为当前 fork 仓库及 GHCR 镜像，移除残留的上游部署入口与过期验收项。
+ [修复] 图片素材文件缺失、图片插入失败以及默认图片/文本节点标题支持中英文切换。

## v0.14.1 - 2026-08-06

+ [调整] 合并上游 `v0.14.0`，同步 i18n 中英文国际化、本地 Codex Skill 管理、Agent 全面重构（Codex 模型与推理强度选择、三档权限模式、任务进度卡片）、提示词库自定义来源与画布面板「提示词」Tab 等功能。
+ [调整] 保留当前 fork 的 GHCR 镜像、Docker Compose、GitHub 链接、版本检查配置和官方插件注册表配置。
+ [调整] 保留 fork 独有补丁：图片素材 `storageKey` 空 URL 解析、`@` 引用覆盖层标签渲染、素材插入完整元信息；IME 输入法修复与上游实现对齐，直接采用上游版本。
...
+ [调整] 合并上游 `v0.9.0`，同步全局 Agent、画布插件系统、面板调整、资产管理和画布导出等功能。
+ [调整] 保留当前 fork 的 GHCR 镜像、Docker Compose、GitHub 链接和版本检查配置，并将官方插件注册表切换到当前仓库。
+ [修复] 迁移图片素材 `storageKey` 解析逻辑，避免素材 URL 为空时插入空图片节点。
+ [修复] 采用上游新版提示词输入组件和原生插入光标修复。
## v0.9.0 - 2026-07-17
...
+ [修复] 将资源引用输入框插入光标修复迁移到 Vite 画布组件。
...
+ [新增] 渠道兼容Gemini格式。
+ [新增] 新增Codex App插件支持。
+ [调整] Docker镜像发布支持fork仓库生成自己的GHCR镜像。
+ [修复] 修复画布节点下方输入框看不到文字插入光标的问题。

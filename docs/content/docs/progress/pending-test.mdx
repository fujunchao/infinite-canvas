...
- 合并上游 `v0.14.0`：前端右上角可切换中英文并持久化（导航、首页、配置中心、Ant Design 组件语言跟随），需验证切换后各页面文案与状态一致。
- 合并上游 `v0.14.0`：Agent 面板支持查看/创建/编辑/删除/启停/显式调用本地 Codex Skill，并可从对话或画布生成 Skill 草稿；需验证 Skill 列表同步、草稿保存与调用流程。
- 合并上游 `v0.14.0`：Agent 支持选择 Codex 模型与推理强度（含 max/ultra）、三档权限模式、任务进度卡片、工具确认模式移至输入框并默认自动确认；需验证选择持久化与各模式行为。
- 合并上游 `v0.14.0`：画布左侧面板新增「提示词」Tab（可折叠提示词来源）；提示词中心改为双栏布局，支持自定义 JSON 来源与独立缓存更新；需验证面板与提示词库交互。
- 合并上游 `v0.14.0`：Canvas Agent 目录重构、升级 Codex 0.146.0，网页侧边栏连接本地 Agent 后需验证对话、工具确认、画布操作与历史线程均正常。
- 合并上游 `v0.14.0`：fork 独有补丁保留（storageKey 空 URL 素材插入、`@` 引用覆盖层标签渲染、素材插入完整元信息），需回归验证素材插入与 `@` 引用行为与合并前一致。
...
- 官方插件发布：`v0.9.1` Tag 应触发 `Publish Official Plugins`，将清单和插件产物发布到当前 fork 的 `plugins-dist` 分支，前端默认从 `fujunchao/infinite-canvas` 的 jsDelivr 地址加载。
...
- Docker 镜像发布：主应用和文档站 workflow 按当前仓库 owner 推送 GHCR，主应用同时发布 `latest` 标签。
- Docker 部署文档：README、主应用 Compose 和文档站 Compose 使用 `ghcr.io/fujunchao/infinite-canvas` 与 `ghcr.io/fujunchao/infinite-canvas-docs`。
- 画布素材插入：图片素材只有 `storageKey`、运行时 `dataUrl` 为空时，仍能恢复本地 Blob 并正常创建图片节点。
- 上游架构合并后的左侧「资产」Tab 与资产选择弹窗均应保留 `storageKey`、原始尺寸和备用 URL，需分别验证 URL 图片和仅有本地 `storageKey` 的图片都能插入画布。
- 画布节点输入框：资源标签覆盖层只在内容实际包含标签时启用，中文输入法组合输入和文字插入光标显示正常。
- 仓库入口：应用顶部 GitHub 图标、版本检查和文档站 GitHub 配置指向当前 fork 仓库。
